"""Structured JSON logger for the dashboard backend.

Adapted from ``capture_api/app/logging.py`` and ``worker/worker/logging.py``
with ``service="dashboard"``. CLAUDE.md "Error handling & logging" requires
the same fields and the same redaction rules across every component.
"""
