...
consumers: dashboard/backend/routers/retrieval.py

This file is intentionally a near-duplicate of telegram_bot/prompts/retrieve.md.
Both surfaces share the same JSON envelope contract, so the retrieval
behaviour must stay identical regardless of whether the question came from
Telegram or the web dashboard. If you change one, mirror the change in the
other in the same PR — the contract is in CLAUDE.md, the prompts are the
implementation.
-->
...
8. Render `answer` as Markdown. The dashboard renders it inside a single
   chat bubble; the Telegram bot chunks at paragraph boundaries.
