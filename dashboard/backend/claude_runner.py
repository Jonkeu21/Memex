"""Subprocess wrapper for ``claude -p`` for the dashboard's retrieval chat.

Mirrors :mod:`telegram_bot.bot.claude_runner` — same envelope shapes, same
inner-JSON validation, same ``ClaudeTimeoutError`` / ``ClaudeTransientError`` /
``ClaudeMalformedJSONError`` taxonomy. Kept duplicated rather than imported to
avoid taking a runtime dependency on the bot package while
``shared/memex_shared/`` extraction remains deferred (see Progress-Tracker.md
...
class ClaudeNotAuthenticatedError(ClaudeTransientError):
    """The ``claude`` CLI exited indicating it's not logged in."""
...
def _validate_inner(inner: Any) -> tuple[str, list[RetrievalSource], list[RetrievalQuote], float]:
    ...
    for i, item in enumerate(sources_raw):
        ...
        if not isinstance(path, str) or not path.strip():
            raise ClaudeMalformedJSONError(
                f"sources[{i}].path must be a non-empty string"
            )
        if path.startswith("/"):
            raise ClaudeMalformedJSONError(
                f"sources[{i}].path must be vault-relative (no leading slash)"
            )
    ...
    for i, item in enumerate(quotes_raw):
        ...
        if idx < 0 or (sources and idx >= len(sources)):
            raise ClaudeMalformedJSONError(
                f"quotes[{i}].source_index out of range: {idx}"
            )
    ...
    if exit_code != 0:
        ...
        if any(marker in lower for marker in _AUTH_ERROR_MARKERS):
            raise ClaudeNotAuthenticatedError(
                "Claude CLI not authenticated; run `claude /login` inside the "
                "claude_auth volume context"
            )
        raise ClaudeTransientError(f"claude -p exited {exit_code}: {stderr}")
